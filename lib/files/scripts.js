...
function init(){
	...
	var asdf = $(window).width();
	var yep = false;
	for (let h = 5; h > -1; h--) {
		if ( yep == false ) {
			if ( asdf > breakpoints[h] ) {
				yep = true;
				selectIcons(h);
			} else if ( asdf < breakpoints[h] && asdf > breakpoints[h-1] ) {
				yep = true;
				selectIcons(h-1);
			} 
		} else {
...
$('#blue icons').children().length


function selectIcons(screenSize) {
	var remove = 12;
	if ( screenSize == 3 ) {
		remove = 9;
